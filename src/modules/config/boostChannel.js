...
export default async function handleSetBoostChannel(itx) {
  if (!itx.memberPermissions.has(PermissionFlagsBits.ManageGuild)) {
    console.warn(`[config/boostChannel] Sin permisos - ${itx.user.tag} en ${itx.guild.name}`);
  ...
  try {
    upsertSettings.run({
      ...
      booster_announce_channel_id: channel.id,
    ...
    console.log(`[config/boostChannel] Canal configurado a ${channel.name} (${channel.id}) en ${itx.guild.name}`);
  } catch (err) {
    console.error(`[config/boostChannel] Error al guardar configuración:`, err.message);
  }
  
  return itx.reply({ content: `Canal de anuncios de boosts seteado a <#${channel.id}>`, ephemeral: true });
}
