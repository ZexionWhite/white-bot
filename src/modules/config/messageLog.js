...
export default async function handleSetMessageLog(itx) {
  if (!itx.memberPermissions.has(PermissionFlagsBits.ManageGuild)) {
    console.warn(`[config/messageLog] Sin permisos - ${itx.user.tag} en ${itx.guild.name}`);
  ...
  try {
    ...
    console.log(`[config/messageLog] Canal configurado a ${channel.name} (${channel.id}) en ${itx.guild.name}`);
  } catch (err) {
    console.error(`[config/messageLog] Error al guardar configuración:`, err.message);
  }
