...
export async function handle(itx) {
  if (!itx.memberPermissions.has(PermissionFlagsBits.ManageGuild)) {
    console.warn(`[settings/avatar-log] Sin permisos - ${itx.user.tag} en ${itx.guild.name}`);
  ...
  try {
    ...
    console.log(`[settings/avatar-log] Canal configurado a ${channel.name} (${channel.id}) en ${itx.guild.name}`);
  } catch (err) {
    console.error(`[settings/avatar-log] Error al guardar configuración:`, err.message);
  }
