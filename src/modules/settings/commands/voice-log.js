...
export async function handle(itx) {
  if (!itx.memberPermissions.has(PermissionFlagsBits.ManageGuild)) {
    console.warn(`[settings/voice-log] Sin permisos - ${itx.user.tag} en ${itx.guild.name}`);
  ...
  try {
    ...
    console.log(`[settings/voice-log] Canal configurado a ${channel.name} (${channel.id}) en ${itx.guild.name}`);
  } catch (err) {
    console.error(`[settings/voice-log] Error al guardar configuración:`, err.message);
  }
